package mobello.amtrust.com.utility;

/**
 * Created by Parthi on 08-Jul-16.
 */
public class WebConstant {

    public static final String SUCCESS = "success";

    public static final String DEFAULT_ACCESSKEY = "xf2MY6iTVxAL0hNO";
    public static final String WEBSERVICE_USRENAME = "ws";

    public static final String  USER_EXIST = "ams.userexist";
    public static final String  GET_CHALLENGE = "getchallenge";
    public static final String  LOGIN = "login";
    public static final String  LOGOUT = "logout";
    public static final String  CREATE = "create";
    public static final String  AMS_LOGOUT = "ams.logout";
    public static final String  AMS_CHANGE_PASSWORD = "ams.changepwd";
    public static final String  CUSTOMER_LOGIN = "ams.login";
    public static final String  CUSTOMER_LOGOUT = "ams.logout";
    public static final String  FORGET_PASSWORD = "ams.forgetpwd";

    public static final String  AMS_COUNTRY_MODULE = "AmsCountry";
    public static final String  CONTACTS_MODULE = "Contacts";


}
